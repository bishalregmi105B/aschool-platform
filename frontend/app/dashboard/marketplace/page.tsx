...
export default function MarketplacePage() {
  ...
  if (isLoading) return <AOSModuleLoadingState label="Loading marketplace…" />;
  ...
  return (
    <AOSPage>
      <AOSPageHeader
        icon={<Store className="h-5 w-5" style={{ color: "var(--w11-accent)" }} />}
        title="App Marketplace & Billing"
        subtitle={
          <>
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "packages" | "plugins")} className="w-full">
          ...
          <TabsContent value="plugins" className="space-y-6">
            ...
            <FilterCommandBar className="mb-0">
              ...
              <div className="flex items-center gap-2 text-sm">
                <button
                  ...
                  className={`win11-chip ${installFilter === "all" ? "accent" : ""}`}
                >
                  All ({allPlugins.length})
                </button>
                <button
                  onClick={() => setInstallFilter("active")}
                  className={`win11-chip ${installFilter === "active" ? "success" : ""}`}
                >
                  Installed &amp; Active ({activeCount})
                </button>
                <button
                  onClick={() => setInstallFilter("not_installed")}
                  className={`win11-chip ${installFilter === "not_installed" ? "accent" : ""}`}
                >
            ...
            <p className="text-sm" style={{ color: "var(--w11-text-secondary)" }}>
              <span className="font-medium" style={{ color: "var(--w11-text-primary)" }}>{activeCount}</span>{" "}
            ...
            {isSearching || installFilter !== "all" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {filtered.length === 0 && (
                  <p className="col-span-3 text-center py-8" style={{ color: "var(--w11-text-secondary)" }}>
              ...
              Object.entries(marketplace).map(([category, plugins]) => (
                <div key={category} className="space-y-4 mb-8">
                  <div className="flex items-center gap-2 border-b border-[var(--w11-border-subtle)] pb-2">
                    {(() => {
                      const Icon = categoryIcons[category] || Layers;
                      return <Icon className="h-5 w-5" style={{ color: "var(--w11-accent)" }} />;
                    })()}
                    <h2 className="text-xl font-semibold" style={{ color: "var(--w11-text-primary)" }}>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {packages.map((pkg) => (
        <div
          key={pkg.key}
          className={`win11-card relative flex flex-col ${pkg.isPopular ? "" : ""}`}
          style={pkg.isPopular ? { borderColor: "var(--w11-accent)", boxShadow: "var(--w11-elevation-flyout)" } : undefined}
        >
          {pkg.isPopular && (
            <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/4">
              <span className="win11-chip accent uppercase text-[10px] tracking-wider">Most Popular</span>
            </div>
          )}
          <div className="p-6 pb-0">
            <div className="flex justify-between items-start mb-4">
              <pkg.icon
                className="h-8 w-8"
                style={{ color: pkg.isPopular ? "var(--w11-accent)" : "var(--w11-text-tertiary)" }}
              />
            </div>
            <h3 className="text-2xl font-bold" style={{ color: "var(--w11-text-primary)" }}>{pkg.title}</h3>
            <p className="h-10 mt-2 text-sm" style={{ color: "var(--w11-text-secondary)" }}>{pkg.description}</p>
          </div>
          <div className="px-6 flex-1">
            <div className="mb-6">
              <span className="text-4xl font-extrabold" style={{ color: "var(--w11-text-primary)" }}>
                NPR {pkg.price.toLocaleString()}
              </span>
              <span className="font-medium" style={{ color: "var(--w11-text-secondary)" }}>/mo</span>
            </div>
            ...
            <ul className="space-y-3">
              {pkg.highlights.map((f: string, i: number) => (
                <li key={i} className="flex items-start gap-3 text-sm">
                  <Check className="h-4 w-4 shrink-0 mt-0.5" style={{ color: "var(--w11-accent)" }} />
                  <span style={{ color: "var(--w11-text-secondary)" }}>{f}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        {notInstalled.length === 0 ? (
          ...
          <div className="space-y-2">
            <p className="text-sm font-medium" style={{ color: "var(--w11-text-primary)" }}>
              Not yet installed ({notInstalled.length}):
            </p>
...
function PluginCard({
  ...
  const cardStyle: React.CSSProperties = isActive
    ? { borderColor: "var(--w11-accent)", boxShadow: "0 0 0 2px var(--w11-accent-light)" }
    : isInactive
      ? { borderColor: "var(--w11-border-strong)" }
      : {};

  return (
    <div className={`win11-card flex flex-col relative transition-all ${""}`} style={cardStyle}>
      {isActive && (
        <div className="absolute -top-2 -right-2">
          <span className="win11-chip success text-[10px] uppercase tracking-wider font-bold">
      ...
      {isInactive && (
        <div className="absolute -top-2 -right-2">
          <span className="win11-chip warning text-[10px] uppercase tracking-wider font-bold">
            Inactive
          </span>
        </div>
      )}
      <div className="p-6 pb-3">
        ...
        <div className="flex items-center gap-2 mt-2">
          {isPaid ? (
            ...
            <StatusChip status="active" label="Free Forever" />
          )}
          {onTrial && (
            <StatusChip
              status="pending"
              label={`Trial${typeof plugin.trial_days_left === "number" ? ` · ${plugin.trial_days_left}d left` : ""}`}
            />
          )}
          {isComingSoon && (
            <StatusChip status="hold" label="Coming Soon" />
          )}
        </div>
      </div>
      <div className="px-6 flex-1">
        <p className="text-sm leading-relaxed" style={{ color: "var(--w11-text-secondary)" }}>{plugin.description}</p>
      ...
      <div
        className="pt-4 border-t gap-2 flex items-stretch"
        style={{ borderColor: "var(--w11-border-subtle)", background: "var(--w11-control-hover)" }}
      >
        {isComingSoon ? (
          // E230: in final testing — install disabled with an explanatory tooltip
          <Button className="w-full m-4" disabled title="In final testing — releasing soon">
            Coming Soon
          </Button>
        ) : isCore && isActive ? (
          // Core plugins are provisioned for every school and cannot be
          // turned off — no lifecycle CTA at all.
          <Button variant="outline" className="w-full m-4" disabled>
          ...
          <>
            {state === "not_installed" && isPaid && (
              <Button className="w-full m-4" onClick={onInstall} disabled={busy}>
            ...
            {isInactive && (
              <>
                <Button className="flex-1 m-4" onClick={onActivate} disabled={busy}>
            ...
            {isActive && onTrial && (
              <>
                <Button className="flex-1 m-4" onClick={onSubscribe} disabled={busy}>
                  {busy ? (
                    <Spinner size="sm" />
                  ) : (
            ...
            {isActive && !isPaid && (
              <Button variant="outline" className="w-full m-4" onClick={onDeactivate} disabled={busy}>
