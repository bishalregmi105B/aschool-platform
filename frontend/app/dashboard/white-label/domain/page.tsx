...
const STATUS_META: Record<string, { label: string; status: string }> = {
  ...
  pending: { label: "Pending Verification", status: "pending" },
  failed: { label: "Verification Failed", status: "failed" },
  not_configured: { label: "Not Configured", status: "hold" },
};
...
function DomainContent() {
  ...
  if (isLoading) return <AOSModuleLoadingState label="Loading domain configuration…" />;
    ...
    return (
      <AOSPage>
        <AOSPageHeader
          ...
          title="Custom Domain"
          subtitle="Point your own domain to this platform"
        />
        <AOSPageBody>
          <DataPanel className="max-w-2xl mx-auto">
            <div className="flex flex-col items-center gap-3 pt-6 text-center">
              ...
              <p className="text-sm" style={{ color: "var(--w11-text-secondary)" }}>
                Failed to load domain configuration. Please try again.
              </p>
      ...
      <AOSPageBody>
        <div className="space-y-4">
          <DataPanel
            ...
            actions={<StatusChip status={statusMeta.status} label={statusMeta.label} />}
          >
            <div className="space-y-4">
            ...
            <DataPanel
              ...
              actions={
                <Button variant="outline" onClick={() => verify.mutate()} disabled={verify.isPending}>
                  {verify.isPending ? <Spinner /> : "Verify DNS"}
                </Button>
              }
            >
              <div>
                ...
                <div className="space-y-3">
                  {dnsRecords.map((rec: any, idx: number) => (
                    <div
                      key={idx}
                      className="grid grid-cols-3 gap-4 p-3 border border-[var(--w11-border-default)] rounded-lg text-sm"
                      style={{ fontFamily: "var(--w11-font-mono)", color: "var(--w11-text-primary)" }}
                    >
                      <div><span className="text-xs block" style={{ color: "var(--w11-text-secondary)" }}>Type</span>{rec.type}</div>
                      <div><span className="text-xs block" style={{ color: "var(--w11-text-secondary)" }}>Name/Host</span>{rec.name}</div>
                      <div><span className="text-xs block" style={{ color: "var(--w11-text-secondary)" }}>Value</span><span className="break-all">{rec.value}</span></div>
                    </div>
                  ))}
                </div>
                {lastVerify && (
                  <div
                    className={`win11-infobar mt-4 ${lastVerify.verdict === "verified" ? "success" : lastVerify.verdict === "failed" ? "error" : "warning"}`}
                  >
                    <div className="space-y-1">
                      ...
                      {lastVerify.records && (
                        <p className="text-xs" style={{ fontFamily: "var(--w11-font-mono)" }}>
          ...
          <div className="win11-infobar warning">
            ...
            <div className="text-sm space-y-1">
              ...
              <p style={{ color: "var(--w11-text-secondary)" }}>
                Verification performs a live DNS lookup of your domain. SSL certificate will be auto-provisioned once DNS is verified.
              </p>
            </div>
          </div>
        </div>
