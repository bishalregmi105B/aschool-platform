  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold">Accommodation Finder</h1><p className="text-muted-foreground">Adjust the environment, not the child — never a diagnosis</p></div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle>Describe the barrier</CardTitle></CardHeader>
        ...
        <Card>
          ...
          <CardContent>
            {gen.isPending ? <PageLoader /> : error ? (
              ...
              <div className="text-center py-16 text-muted-foreground"><Accessibility className="h-12 w-12 mx-auto mb-4 opacity-50" /><p>Describe the barrier — get practical classroom adjustments.</p></div>
            ) : (
              <div className="space-y-3">
                {result.accommodations?.map((a, i) => (
                  <div key={i} className="rounded-md border p-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold capitalize text-muted-foreground">{(a.category || "support").replace(/_/g, " ")}</p>
                    </div>
                    <p className="text-sm mt-1"><span className="font-medium">Barrier:</span> {a.barrier}</p>
                    <p className="text-sm"><span className="font-medium">Try:</span> {a.accommodation}</p>
                  </div>
                ))}
                {result.summary && (
