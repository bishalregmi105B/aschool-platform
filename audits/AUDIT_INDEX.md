...
| :--- | :--- | :--- | :--- |
| **2026-09-05** | [**`research/ROADMAP_V5_2026-09-05.md`**](research/ROADMAP_V5_2026-09-05.md) | **Roadmap v5 — PLAN OF RECORD.** Integration-first correction from the W4 wave: ATeacher is integrated as the EXTERNAL Ashlya service (~150-line shim, no engine rebuild), parity lock is Phase P before any theme work, tool catalog waves follow claude-skills discipline, all-screens + plugin-arch audits folded into phases A′/W. | Full Stack, Plan |
| **2026-09-05** | [`research/W4/A1_ATEACHER_SERVICE_CONTRACT.md`](research/W4/A1_ATEACHER_SERVICE_CONTRACT.md) | The REAL ATeacher service API (routes/auth/sessions/sockets/TTS), deploy requirements (eventlet/6001/MySQL), Ashlya's 2-proxy-call host flow. | ATeacher Integration |
| **2026-09-05** | [`research/W4/A2_INTEGRATION_GAP_LIST.md`](research/W4/A2_INTEGRATION_GAP_LIST.md) | Field-by-field mismatch list between ASchool's service_client contract and the real service; shim spec; player embedding + CSP; webhook-secrets hole fix. | ATeacher Integration |
| **2026-09-05** | [`research/W4/A3_WHAT_TO_KEEP_FROM_HOST_BUILD.md`](research/W4/A3_WHAT_TO_KEEP_FROM_HOST_BUILD.md) | Which host-side pieces stay (S2S client, webhooks, models, tasks, streaming) and what was wrongly rebuilt (removed). | ATeacher Integration |
| **2026-09-05** | [`research/W4/B1_CLAUDE_SKILLS_INVENTORY.md`](research/W4/B1_CLAUDE_SKILLS_INVENTORY.md) | glebis/claude-skills study → 10 skill mappings + 12 prompt-file patterns (trigger phrases, progressive disclosure, budgets, failure modes, dual verify). | AI Tools |
| **2026-09-05** | [`research/W4/B2_INTERNATIONAL_TOOL_LANDSCAPE.md`](research/W4/B2_INTERNATIONAL_TOOL_LANDSCAPE.md) | 8 platforms (MagicSchool/Diffit/Twee/Curipod/Gamma/Quizizz/Brisk…) + 62-row merged tool table with ASchool engine mapping. | AI Tools, Competitors |
| **2026-09-05** | [`research/W4/B3_ENGINE_POWERED_TOOLS.md`](research/W4/B3_ENGINE_POWERED_TOOLS.md) | Specs: presentation generator (native PPTX), worksheet/paper renderer, rubric/batch feedback, notes-assist (no OCR); engine upgrades ranked; catalog waves 2-5. | AI Tools, Engines |
| **2026-09-05** | [`research/W4/C1_ALL_SCREENS_AUDIT.md`](research/W4/C1_ALL_SCREENS_AUDIT.md) | Census of all 224 web pages (91 complete / 102 basic / 7 thin), 8 daily-flow traces, Flutter screen audit + release blockers, 10 adoptable UI patterns, top-20 fix list. | UI/UX, Web+Flutter |
| **2026-09-05** | [`research/W4/D1_PLUGIN_ARCH_STATE.md`](research/W4/D1_PLUGIN_ARCH_STATE.md) | Plugin v2 engine built-but-unadopted (0/41 manifests), widget customization gaps (no dashboard_layouts), parity lock missing, template-pack spec, mobile manifest path; 10 ranked builds. | Plugin Architecture, Themes |
| **2026-09-05** | [`research/ROADMAP_V4_2026-09-05.md`](research/ROADMAP_V4_2026-09-05.md) | Roadmap v4 — Phase C build order superseded by v5 (§0); Phase 0/B′ results and the rest remain the verified basis. | Full Stack, Plan |
...
### [2026-09-05] - W0 REPAIR WAVE: plugin-registry dedup, dead-plugin deletion, manifest-pointer validator, silent-gap wiring
- **Author/Agent:** ZCode implementation agent (UNIFIED_ROADMAP v2 §5 "W0 — Repair & wire")
- **Rationale:** The 13-report research wave established that the backend is ~92% complete but the plugin registry had a split-brain AI gate, 4 dead plugins still billable in the marketplace, ~20 manifest code pointers aimed at files that do not exist, and several fully-implemented backend paths with no consumer at all (the daily absent-student cron emitted an event nobody listened to). W0 clears that debt so W1–W6 build on a registry that tells the truth.
- **Action Taken:**
  - **AI gate consolidation:** 10 frontend `PluginGate` slugs (`ai_tools`/`ai_adaptive_learning`/`benchmarking`/`ai_insights`/`ai_grading`/`ai_tutor`) repointed to `ai_suite`; 7 deprecated AI manifests delisted; `ai_suite` made a real Python package (it had no `__init__.py`) and inherited the `config_schema.yaml` from the removed `ai_tools` folder.
  - **Dead-plugin deletion:** `social_ads`, `social_hub`, `digital_content`, `advanced_analytics` modules + `api/v1/social_{ads,hub}.py` + `services/social/` (5 files) + `tasks/social_{scheduler,sync}.py` + `models/{social,ad_campaign}.py` + the `social-publish-scheduled` beat entry, ~1,280 lines net removed. Migration `e8b1c4d6a9f2` drops the 8 orphaned tables children-first.
  - **`grant_plan_plugins` no longer grants `coming_soon`/`deprecated` plugins** (`entitlements.py` now consults the manifest) — conferences/gps_tracking/whatsapp_bot were installable free on any plan.
  - **Manifest-pointer validator** (`loader.py::_validate_pointers`, fail-soft with one ERROR log per broken pointer, path resolution anchored to the backend root so it is CWD-independent): found **19 real breakages**, all fixed (e.g. `fees` esewa→`esewa_gateway`, `exams` report_card_generator→`report_generation`, `timetable` timetable_generator→`timetable_solver`, `wellbeing` sentiment_analysis→`school_insights`). Validator now reports 0.
  - **Silent-gap wiring:** `attendance.student_absent` finally has a consumer (`plugins/listeners.py` → guardian push + SMS + in-app rows); leave **approval writes `TeacherAttendance` rows** across the leave's date range and **rejection persists its reason** (new `leave_requests.rejection_reason`, migration `d5c8f2a7b4e1`); 3 read-only curriculum endpoints added (`/academics/curriculum/frameworks`, `…/<id>` with nested units+outcomes, `/academics/subject-offerings`) which W3's AI Teacher grounding depends on.
  - **Benchmarking rewrite:** `/rankings` replaced a per-school N+1 loop with set-based grouped queries + a 600 s cache, moved its gate to `ai_suite`, and **stopped returning other schools' names** (it was a cross-tenant dump).
  - **Ops hardening:** Sparrow SMS `http://`→`https://` in both call sites; `device_rate_limit()` (SHA-256 of `X-Device-Key`, 120/min) on the unauthenticated biometric ingest route; `python-docx==1.1.*` declared in requirements (it was imported by `services/writer_docx.py` but never installed, so PDF/DOCX endpoints 501'd in the production image).
  - **Tooling:** `backend/scripts/migrate.py` — a programmatic Alembic driver, because Flask-Migrate 4.0.7 in this venv registers no `flask db` CLI entry point.
- **Verification:** migration chain replayed from an **empty database** to head `e8b1c4d6a9f2`; pytest **192 passed / 0 failed** across `test_plugin_registry`, `test_comms_plugins`, `test_plugins`, `test_plugin_manifests`, `test_plugin_aliases`, `test_models`, `test_exam_attendance_timetable_assignment`, `test_api`, `test_marketplace_billing`, `test_audit_and_honesty`; `tsc --noEmit` clean. Two registry-test edits were needed: the `config_schema` "no schema" probe moved `library_management`→`alumni` (a **pre-existing** failure proven with `git stash` — `library_management` gained a schema in `3d327eb`), and `plugins_nav` was restored to `DELISTED_SLUGS`.
- **Audit References:** `audits/research/UNIFIED_ROADMAP_2026-09.md` §5, `audits/research/ASCHOOL_PLUGIN_DUPLICATION_AUDIT.md`, `audits/research/ASCHOOL_BACKEND_COMPLETENESS_AUDIT.md`.
