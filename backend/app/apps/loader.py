...
class AppLoader:
    ...
    def alias_map(cls) -> dict[str, str]:
        ...
        """
        from app.apps.decorators import PLUGIN_SLUG_ALIASES
    ...
    def get_frontend_sidebar(cls, installed_slugs: list[str], user_role: str) -> list:
        ...
        for slug in all_slugs:
            ...
            for item in nav.get("subitems") or []:
                # Per-subitem role gating: an app visible to parents can keep
                # admin-only subroutes (e.g. Fees → "Fee Types") out of their
                # nav. Subitems without visible_to remain visible to everyone
                # who can see the app.
                sub_visible = item.get("visible_to") or []
                if (
                    sub_visible
                    and "all" not in sub_visible
                    and user_role not in sub_visible
                ):
                    continue
